              ...
              <table className="w-full border-collapse text-sm">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-slate-500 whitespace-nowrap w-24">Total Linen Rewash</th>
                    <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-emerald-600 whitespace-nowrap w-20">Bersih</th>
                    <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-rose-500 whitespace-nowrap w-20">Kotor</th>
                  ...
                  {loading ? (
                    ...
                    sortedGroups.map((g, idx) => (
                      <tr key={`${g.report_date}|${g.hospital_id}|${g.reporter_name}`}
                        ...
                        {/* Bersih */}
                        <td className="px-4 py-3.5 text-center">
                          <span className="inline-flex items-center justify-center h-6 min-w-6 rounded-full bg-emerald-50 px-2 text-xs font-bold text-emerald-700 tabular-nums">
                            {g.items.reduce((s, i) => s + (i.clear ?? 0), 0)}
                          </span>
                        </td>
                        {/* Kotor = qty - clear */}
                        <td className="px-4 py-3.5 text-center">
                          <span className="inline-flex items-center justify-center h-6 min-w-6 rounded-full bg-rose-50 px-2 text-xs font-bold text-rose-500 tabular-nums">
                            {g.items.reduce((s, i) => s + Math.max(0, (i.qty || 0) - (i.clear ?? 0)), 0)}
                          </span>
                        </td>
