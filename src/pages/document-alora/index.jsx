...
const MENU_ITEMS = [
  {
    ...
    description: "Daftar dokumen perusahaan",
    end: true,
  },
  {
    to: "/document-alora/transactions",
    icon: HiOutlineArrowsRightLeft,
    label: "Peminjaman",
    description: "Pinjam & kembalikan dokumen",
  },
];
      ...
      <div className={cn("flex items-center border-b border-slate-100 py-4",
        ...
        {!collapsed && (
          <div className="flex min-w-0 flex-1 items-center gap-3">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-blue-900 to-blue-700 shadow-md">
              <HiOutlineClipboardDocumentList className="h-5 w-5 text-white" />
            </div>
            <div className="min-w-0">
              <p className="text-sm font-bold text-slate-800 leading-tight truncate">Document Alora</p>
              <p className="text-[11px] text-slate-400 truncate">Manajemen Dokumen</p>
            </div>
          </div>
        )}
        {collapsed && (
...
function ActiveMenuTitle() {
  ...
  const label       = active?.label       ?? "Document Alora";
  const description = active?.description ?? "Manajemen Dokumen Perusahaan";
  const Icon        = active?.icon        ?? HiOutlineClipboardDocumentList;
  return (
    <div className="flex items-center gap-2.5">
      <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-50 border border-blue-200 text-blue-900 shrink-0">
        <Icon className="h-4 w-4" />
      </div>
      <div>
...
export default function DocumentAlora() {
  const [mobileOpen, setMobileOpen] = useState(false);
  const [desktopCollapsed, setCollapsed] = useState(false);
  const drawerRef = useRef(null);

  useEffect(() => {
    document.title = "Document Alora | Alora Group Indonesia";
    ...
    <div className="flex h-screen overflow-hidden bg-slate-100/70">
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <header className="hidden lg:flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4">
          ...
          <div className="flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-900">
        ...
        <header className="flex items-center gap-3 border-b border-slate-200 bg-white px-4 py-3 lg:hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-slate-700">Document Alora</span>
          </div>
        </header>
