...
const MENU_SECTIONS = [
    ...
    items: [
      {
        ...
        label: "Dashboard KMP",
        description: "Transaksi & rekap KMP",
        end: true,
      },
      {
        to: "/event-b2b-alora/customer",
        icon: HiOutlineUserGroup,
        label: "Customer KMP",
        description: "Daftar customer koperasi",
        end: true,
      },
    ],
  },
  {
    ...
    items: [
      {
        to: "/event-b2b-alora/haji",
        icon: HiOutlineBuildingStorefront,
        label: "B2B Haji",
        description: "Transaksi & rekap Haji",
        end: true,
      },
    ],
  },
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className={cn(
        ...
        {!collapsed && (
          <div className="flex min-w-0 flex-1 items-center gap-3">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl
              bg-gradient-to-br from-sky-600 to-cyan-500 shadow-md">
              <HiOutlineBuildingStorefront className="h-5 w-5 text-white" />
            </div>
            <div className="min-w-0">
              <p className="truncate text-sm font-bold text-slate-800 leading-tight">B2B Alora Group</p>
              <p className="truncate text-[11px] text-slate-400">Event & Transaction</p>
            </div>
          </div>
        )}
        {collapsed && (
        ...
        {onClose && (
          <button type="button" onClick={onClose}
            className="flex shrink-0 items-center justify-center rounded-lg p-1.5 text-slate-400
              hover:bg-slate-100 hover:text-slate-600 transition lg:hidden"
      ...
      {MENU_SECTIONS.map((section) => (
        <div key={section.title}>
          {!collapsed && (
            <p className={cn(
              "px-5 pb-1.5 text-[10px] font-bold uppercase tracking-widest text-slate-400",
              "first:pt-4 pt-3",
            )}>{section.title}</p>
          )}
...
function ActiveMenuTitle() {
  ...
  const label       = active?.label       ?? "B2B Alora";
  const description = active?.description ?? "Event & Transaction";
  const Icon        = active?.icon        ?? HiOutlineBuildingStorefront;

  return (
    <div className="flex items-center gap-2.5">
      <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-sky-50 border border-sky-200 text-sky-700 shrink-0">
        <Icon className="h-4 w-4" />
      </div>
      <div>
...
export default function B2BAlora() {
  ...
  useEffect(() => {
    document.title = "B2B Alora Group | Alora Group Indonesia";
  }, []);

  return (
    <div className="flex h-screen overflow-hidden bg-sky-50/50">
      {/* Desktop sidebar */}
      <aside className={cn(
        ...
        <header className="flex items-center gap-3 border-b border-slate-200 bg-white px-4 py-3 lg:hidden">
          ...
          <div className="flex items-center gap-2">
            <div className="flex h-7 w-7 items-center justify-center rounded-lg
              ...
              <HiOutlineBuildingStorefront className="h-3.5 w-3.5 text-white" />
            </div>
            <span className="text-sm font-bold text-slate-700">B2B Alora</span>
          </div>
        </header>
