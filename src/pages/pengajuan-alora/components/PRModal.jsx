        ...
        <div className="bg-white text-slate-800 font-sans">
            ...
            <div className="grid grid-cols-2 gap-8 mb-6">
                <div className="space-y-3">
                    ...
                    <div><div className="text-[10px] text-slate-400 uppercase">Kategori</div><div className="text-sm font-semibold text-slate-700">{toTitleCase(d.company_name)}</div>{(d.outlet_name || Number(d.company_id) === 5) && <div className="text-xs text-slate-500">Outlet: {d.outlet_name || "Seluruh Outlet"}</div>}</div>
            ...
            {/* Jika GA (is_routine): hanya 1 kolom Diajukan Oleh, tanpa Supervisor */}
            {d.is_routine ? (
                <div className="mt-6 flex justify-end">
                    <div className="border border-slate-200 rounded-xl p-4 text-center w-64">
                        ...
                        <div className="text-sm font-semibold text-slate-700">{toTitleCase(d.ga_name || d.pengaju_name)}</div>
...
<div class="grid-2">
  <div class="space-y"><div class="section-title">Diajukan Oleh</div><div class="info-item"><div class="info-label">Nama</div><div class="info-value">${toTitleCase(d.pengaju_name)}</div>${d.employee_code ? `<div class="info-code">${d.employee_code}</div>` : ""}</div><div class="info-item"><div class="info-label">Departemen</div><div class="info-value">${toTitleCase(d.department_name)}</div></div><div class="info-item"><div class="info-label">Kategori</div><div class="info-value">${toTitleCase(d.company_name)}</div>${(d.outlet_name || Number(d.company_id) === 5) ? `<div class="info-sub">Outlet: ${d.outlet_name || "Seluruh Outlet"}</div>` : ""}</div><div class="info-item"><div class="info-label">Tanggal Pengajuan</div><div class="info-value">${formatDate(d.tanggal_pengajuan)}</div></div></div>
...
<div class="grid-2" style="margin-top:24px;gap:16px">${d.is_routine ? `<div style="grid-column:1/-1;display:flex;justify-content:flex-end"><div class="sig-box" style="width:240px"><div class="sig-label">Diajukan Oleh</div><div class="sig-space"></div><div class="sig-name">${toTitleCase(d.ga_name || d.pengaju_name)}</div><div class="sig-role">General Affair</div><div class="sig-date">${formatDateShort(d.approved_ga_at || d.tanggal_pengajuan)}</div></div></div>` : (d.ga_name && !d.spv_name ? `<div style="grid-column:1/-1;display:flex;justify-content:flex-end"><div class="sig-box" style="width:240px"><div class="sig-label">Diajukan & Disetujui GA</div><div class="sig-space"></div><div class="sig-name">${toTitleCase(d.pengaju_name)}</div><div class="sig-role">General Affair</div><div class="sig-date">${formatDateShort(d.approved_ga_at || d.tanggal_pengajuan)}</div></div></div>` : `<div class="sig-box"><div class="sig-label">Diajukan Oleh</div><div class="sig-space"></div><div class="sig-name">${toTitleCase(d.pengaju_name)}</div><div class="sig-role">${getJobLabel(d.pengaju_job_level)}</div><div class="sig-date">${formatDateShort(d.tanggal_pengajuan)}</div></div><div class="sig-box"><div class="sig-label">Disetujui Supervisor</div><div class="sig-space"></div>${d.spv_name ? `<div class="sig-name">${toTitleCase(d.spv_name)}</div><div class="sig-date">${formatDateShort(d.approved_spv_at)}</div>` : `<div class="sig-date" style="color:#d97706;font-style:italic">Menunggu</div>`}</div>`)}</div>
