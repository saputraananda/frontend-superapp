...
const KATEGORI = ["Transport", "Makan & Minum", "Perlengkapan Kantor", "Komunikasi", "Akomodasi", "Lainnya"];
...
function StatusBadge({ status }) {
    ...
    return <span className={cn("inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-semibold", c.cls)}>{c.label}</span>;
}
...
const EMPTY_FORM = { judul: "", deskripsi: "", kategori: "Transport", tanggal_pengeluaran: "", jumlah_nominal: "", bukti_keterangan: "" };
...
export default function Reimburse() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            const d = await api(`/pengajuan/reimburse?${params}`);
            setData(d.data || []);
            setTotal(d.total || 0);
        } catch (err) {
    ...
    const openEdit = (row) => {
        ...
        setForm({
            ...
            kategori: row.kategori || "Transport",
            tanggal_pengeluaran: row.tanggal_pengeluaran?.split("T")[0] || "",
            jumlah_nominal: row.jumlah_nominal || "",
            bukti_keterangan: row.bukti_keterangan || "",
        });
        setModalOpen(true);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (!form.tanggal_pengeluaran) return showToast("error", "Tanggal pengeluaran wajib diisi");
        if (!form.jumlah_nominal) return showToast("error", "Nominal wajib diisi");
        setSaving(true);
        try {
            if (editTarget) {
                await api(`/pengajuan/reimburse/${editTarget.id}`, { method: "PUT", body: JSON.stringify(form) });
                showToast("success", "Reimburse berhasil diperbarui");
            } else {
                await api("/pengajuan/reimburse", { method: "POST", body: JSON.stringify(form) });
                showToast("success", "Reimburse berhasil diajukan");
            }
            setModalOpen(false);
            load();
        } catch (err) {
    ...
    const handleSubmitApproval = async (id) => {
        try {
            await api(`/pengajuan/reimburse/${id}/submit`, { method: "POST" });
            showToast("success", "Reimburse dikirim untuk approval");
            load();
        ...
        <div className="p-6 space-y-5">
            ...
            <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-lg font-bold text-slate-800">Reimburse</h1>
                        <p className="text-xs text-slate-500">Pengajuan pengembalian biaya operasional</p>
                    </div>
                </div>
                <button onClick={openAdd}
                    ...
                    <span className="hidden sm:inline">Ajukan Reimburse</span>
                </button>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                    ...
                    <input value={searchInput} onChange={(e) => setSearchInput(e.target.value)}
                        placeholder="Cari judul reimburse..."
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
                        ...
                        <p className="text-sm text-slate-400">Belum ada pengajuan reimburse</p>
                    </div>
                ) : (
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-100">
                                    ...
                                    <th className="px-5 py-3.5 text-left text-[11px] font-bold text-slate-400 uppercase tracking-widest hidden sm:table-cell">Kategori</th>
                                    <th className="px-5 py-3.5 text-left text-[11px] font-bold text-slate-400 uppercase tracking-widest hidden md:table-cell">Nominal</th>
                                    <th className="px-5 py-3.5 text-left text-[11px] font-bold text-slate-400 uppercase tracking-widest hidden lg:table-cell">Tgl Pengeluaran</th>
                                ...
                                {data.map((row) => (
                                    <tr key={row.id} className="hover:bg-emerald-50/30 transition-colors">
                                        ...
                                        <td className="px-5 py-4 hidden sm:table-cell">
                                            <span className="text-sm text-slate-600">{row.kategori}</span>
                                        </td>
                                        <td className="px-5 py-4 hidden md:table-cell">
                                            <span className="text-sm font-semibold text-emerald-700">{formatRupiah(row.jumlah_nominal)}</span>
                                        </td>
                                        <td className="px-5 py-4 hidden lg:table-cell">
                                            <span className="text-sm text-slate-500 tabular-nums">{row.tanggal_pengeluaran?.split("T")[0] ?? "—"}</span>
                                        </td>
                ...
                <div className="fixed inset-0 z-[9998] flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden border border-slate-200">
                        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4 flex-shrink-0 bg-white">
                            <h2 className="text-base font-bold text-slate-800">{editTarget ? "Edit Reimburse" : "Ajukan Reimburse Baru"}</h2>
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
                            <div className="overflow-y-auto flex-1 p-6 space-y-4 bg-slate-50/40">
                                <div>
                                    ...
                                    <input className={inputCls} value={form.judul} onChange={e => setForm(p => ({ ...p, judul: e.target.value }))} placeholder="Contoh: Tiket perjalanan dinas..." />
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Deskripsi</label>
                                    <textarea rows={2} className={cn(inputCls, "resize-none")} value={form.deskripsi} onChange={e => setForm(p => ({ ...p, deskripsi: e.target.value }))} placeholder="Keterangan tambahan..." />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Kategori</label>
                                        <select className={inputCls} value={form.kategori} onChange={e => setForm(p => ({ ...p, kategori: e.target.value }))}>
                                            {KATEGORI.map(k => <option key={k}>{k}</option>)}
                                        </select>
                                    </div>
                                    <div>
                                        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Tanggal Pengeluaran <span className="text-rose-500">*</span></label>
                                        <input type="date" className={inputCls} value={form.tanggal_pengeluaran} onChange={e => setForm(p => ({ ...p, tanggal_pengeluaran: e.target.value }))} />
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Nominal (Rp) <span className="text-rose-500">*</span></label>
                                    <input type="number" min={0} className={inputCls} value={form.jumlah_nominal} onChange={e => setForm(p => ({ ...p, jumlah_nominal: e.target.value }))} placeholder="0" />
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Keterangan Bukti</label>
                                    <input className={inputCls} value={form.bukti_keterangan} onChange={e => setForm(p => ({ ...p, bukti_keterangan: e.target.value }))} placeholder="No. struk, nama merchant, dll." />
                                </div>
                            </div>
