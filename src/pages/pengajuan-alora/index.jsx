...
const MENU_ITEMS = [
    {
        ...
        description: "Ajukan kebutuhan pengadaan",
        end: true,
    },
    {
        to: "/pengajuan-reimburse",
        icon: HiOutlineCreditCard,
        label: "Reimburse",
        description: "Pengajuan reimbursement biaya",
    },
];
                ...
                <>
                    ...
                    {!collapsed && (
                        <div className="min-w-0 flex-1 overflow-hidden">
                            ...
                            <p className={cn("mt-0.5 truncate text-[11px] leading-none", isActive ? "text-emerald-100" : "text-slate-400")}>
                                {description}
                            </p>
                        </div>
                    )}
...
function Sidebar({ collapsed = false, onClose }) {
    ...
    return (
        <div className="flex h-full flex-col overflow-hidden">
            {/* Brand header */}
            <div className={cn("flex items-center border-b border-slate-100 py-4", collapsed ? "justify-center px-2" : onClose ? "justify-between px-5 gap-3" : "px-5")}>
                {!collapsed && (
            ...
            <div className={cn("border-t border-slate-100 py-3", collapsed ? "px-1.5" : "px-3")}>
                <button type="button" title={collapsed ? "Kembali ke Portal" : undefined}
                    ...
                    className={cn("group relative flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-500 transition hover:bg-slate-100 hover:text-slate-800", collapsed && "justify-center px-2")}>
...
function ActiveMenuTitle() {
    ...
    const description = active?.description ?? "Pengajuan & Reimbursement";
...
export default function PengajuanAlora() {
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-emerald-50/40">
            {/* Desktop sidebar */}
            <aside className={cn("hidden lg:flex shrink-0 flex-col border-r border-slate-200 bg-white shadow-sm transition-[width] duration-300 ease-in-out overflow-hidden", desktopCollapsed ? "w-20" : "w-64")}>
                <Sidebar collapsed={desktopCollapsed} />
            </aside>

            {/* Mobile overlay */}
            <div aria-hidden="true"
                className={cn("fixed inset-0 z-40 bg-black/50 backdrop-blur-sm transition-opacity duration-300 lg:hidden", mobileOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none")}
                onClick={() => setMobileOpen(false)} />

            {/* Mobile drawer */}
            <aside ref={drawerRef} aria-label="Sidebar navigasi"
                className={cn("fixed inset-y-0 left-0 z-50 w-72 flex flex-col border-r border-slate-200 bg-white shadow-2xl transition-transform duration-300 ease-in-out lg:hidden", mobileOpen ? "translate-x-0" : "-translate-x-full")}>
