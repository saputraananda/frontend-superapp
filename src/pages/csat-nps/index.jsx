...
const MENU_ITEMS = [
  {
    to: "/result-of-our-work/waschen",
    icon: HiOutlineSparkles,
    label: "Waschen Laundry",
    description: "CSAT & NPS Waschen",
    end: true,
  },
  {
    to: "/result-of-our-work/cleanox",
    icon: HiOutlineStar,
    label: "Cleanox",
    description: "CSAT & NPS Cleanox",
    end: true,
  },
];
...
function Sidebar({ collapsed = false, onClose }) {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      <div className={cn(
        ...
        {!collapsed && (
          <div className="flex min-w-0 flex-1 items-center gap-3">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-amber-500 to-orange-500 shadow-md">
              <HiOutlineStar className="h-5 w-5 text-white" />
            </div>
            <div className="min-w-0">
              <p className="truncate text-sm font-bold text-slate-800 leading-tight">CSAT & NPS</p>
              <p className="truncate text-[11px] text-slate-400">Result of Our Work</p>
            </div>
          </div>
        )}
        {collapsed && (
      ...
      {!collapsed && (
        <p className="px-5 pt-4 pb-1.5 text-[10px] font-bold uppercase tracking-widest text-slate-400">Unis Bisnis</p>
      )}
...
function ActiveMenuTitle() {
  ...
  const label = active?.label ?? "CSAT & NPS";
  const description = active?.description ?? "Customer Satisfaction & NPS";
  const Icon = active?.icon ?? HiOutlineStar;

  return (
    <div className="flex items-center gap-2.5">
      <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-amber-50 border border-amber-200 text-amber-600 shrink-0">
        <Icon className="h-4 w-4" />
      </div>
      <div>
...
export default function CsAtNps() {
  ...
  useEffect(() => {
    document.title = "CSAT & NPS — Result of Our Work | Alora Group Indonesia";
  }, []);
        ...
        <header className="flex items-center gap-3 border-b border-slate-200 bg-white px-4 py-3 lg:hidden">
          ...
          <div className="flex items-center gap-2">
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-amber-500 to-orange-500">
              <HiOutlineStar className="h-3.5 w-3.5 text-white" />
            </div>
            <span className="text-sm font-bold text-slate-700">CSAT & NPS</span>
          </div>
        </header>
