...
import LiburKaryawanCleanox from "./LiburKaryawanCleanox.jsx";
import {
	HiOutlineArrowLeft,
...
// ─── Main page ────────────────────────────────────────────────────────────────
export default function PerizinanCleanox() {
	...
	const fetchLeaves = useCallback(
		async ({ silent = false } = {}) => {
			...
			try {
				...
				const data = await api(`/cleanox/leaves?${qs.toString()}`);
				const records = (data.records ?? []).map((row) => ({
					...row,
					doctor_note_url: resolveAssetUrl(row.doctor_note_url),
				}));
				setRecords(records);
				setPagination((prev) => ({
					...prev,
	...
	useEffect(() => {
		document.title = "Cuti & Perizinan Cleanox | Alora Group Indonesia";
	}, []);
	...
	const handleApprove = async (_note) => {
		...
		setActionBusy(true);
		try {
			await api(`/cleanox/leaves/${actionModal.item.id}/approve`, {
				method: "PUT",
	...
	const handleReject = async (note) => {
		...
		setActionBusy(true);
		try {
			await api(`/cleanox/leaves/${actionModal.item.id}/reject`, {
				method: "PUT",
				body: JSON.stringify({
					rejection_note: note,
		...
		<>
			...
			<main className="min-h-screen bg-slate-50 py-6 sm:py-10">
				<div className="mx-auto max-w-screen-2xl px-4 sm:px-6 lg:px-8 space-y-6">
					...
					{/* ── Header ─────────────────────────────────────────────────── */}
					<section className="relative overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-br from-[#1b3459] via-[#1b3459] to-[#97bd3f] p-5 shadow-sm sm:p-6">
						<div className="absolute -right-20 -top-20 h-64 w-64 rounded-full bg-white/10 blur-3xl" />
						<div className="absolute -left-20 bottom-0 h-56 w-56 rounded-full bg-[#97bd3f]/20 blur-3xl" />

						<div className="relative flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
							<div className="max-w-2xl">
								<h1 className="mt-4 text-2xl font-bold text-white sm:text-3xl">Cuti &amp; Perizinan Cleanox</h1>
								<p className="mt-2 text-sm text-white/80">
									Kelola pengajuan izin, sakit, cuti, dan plot libur karyawan.
								</p>
							</div>
					...
					<section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
						...
						<div className="mt-3 inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
							<HiOutlineCalendarDays className="h-4 w-4 text-[#97bd3f] shrink-0" />
							<span>
					...
					<section className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
						<div className="border-b border-slate-100 px-5 py-4 flex items-center justify-between gap-3">
							<div className="flex items-center gap-2">
								<HiOutlineClipboardDocumentList className="h-5 w-5 text-[#97bd3f]" />
									...
									p === "..." ? (
										...
										<button key={p} type="button" onClick={() => handlePage(p)} disabled={loading}
											className={cn(
												...
												p === pagination.page
													? "border-[#1b3459] bg-[#1b3459] text-white shadow-sm"
