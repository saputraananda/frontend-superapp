...
function buildFilterLabel(filters) {
  ...
  const { serviceMode, filterType, month, year, startDate, endDate } = filters;
  const layananLabel =
  ...
  return `${layananLabel}  |  ${periodLabel}`;
}

// ─── Main export function ─────────────────────────────────────────────────────
export function exportPiutangCleanoxExcel({ rows, meta, filters, searchKeyword, statusFilter }) {
  const TOTAL_COLS = 10;
  ...
  const HEADERS = ["#", "Customer", "No. Telepon", "Layanan", "No Nota", "Tgl Terima", "Jatuh Tempo", "Aging (Hari)", "Jumlah Piutang", "Status"];
  ...
  saveAs(
    ...
    `piutang_cleanox_${fileDate.replaceAll("-", "")}.xlsx`
  );
}
