...
export default function CategoryServices() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const res = await api(`/waschen/category-services?${query.toString()}`);
      setData(res.data || []);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.id) {
        await api(`/waschen/category-services/${formData.id}`, {
          method: "PUT",
          body: JSON.stringify(formData),
        });
        showToast("Kategori layanan berhasil diperbarui");
      } else {
        await api("/waschen/category-services", {
          method: "POST",
          body: JSON.stringify(formData),
        });
        showToast("Kategori layanan berhasil ditambahkan");
      }
      setModalOpen(false);
      loadData();
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api(`/waschen/category-services/${deleteTarget.id}`, { method: "DELETE" });
      showToast("Kategori layanan berhasil dihapus");
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
        ...
        <div className="relative p-5 sm:p-6 lg:p-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              <h1 className="text-2xl font-bold tracking-tight text-white sm:text-3xl">Kategori Layanan</h1>
              <p className="mt-3 text-sm leading-6 text-white/75 sm:text-base">
                Pengelolaan grup & kategori jenis layanan POS Waschen Laundry
              </p>
            </div>
            <button
              type="button"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Total Kategori</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Kategori Aktif</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Nonaktif</p>
            <p className="text-2xl font-bold text-slate-400 mt-0.5">{stats.inactive}</p>
          </div>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl overflow-hidden border border-slate-200">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 bg-slate-50/50">
              <h3 className="font-bold text-slate-800 text-sm">
                {formData.id ? "Edit Kategori Layanan" : "Tambah Kategori Layanan"}
              </h3>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs">
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Kode Kategori *</label>
                <input
                  type="text"
                  placeholder="Misal: KILOAN, SATUAN, SEPATU_TAS"
                  value={formData.code}
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="Misal: Layanan Kiloan Reguler"
                  value={formData.name}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-sm rounded-2xl bg-white shadow-2xl p-5 border border-slate-200 space-y-4">
            <div className="flex items-center gap-3 text-rose-600">
              ...
              <h3 className="font-bold text-slate-800 text-sm">Hapus Kategori Layanan?</h3>
            </div>
            <p className="text-xs text-slate-600">
              Apakah Anda yakin ingin menghapus kategori <strong>{deleteTarget.name}</strong> ({deleteTarget.code})? Tindakan ini tidak dapat dibatalkan.
            </p>
