...
export default function Parfume() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const res = await api(`/waschen/parfumes?${query.toString()}`);
      setData(res.data || []);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name.trim()) {
      setFormError("Nama Parfum wajib diisi");
      return;
    }
    ...
    try {
      if (formData.id) {
        await api(`/waschen/parfumes/${formData.id}`, {
          method: "PUT",
          body: JSON.stringify(formData),
        });
        showToast("Parfum berhasil diperbarui");
      } else {
        await api("/waschen/parfumes", {
          method: "POST",
          body: JSON.stringify(formData),
        });
        showToast("Parfum berhasil ditambahkan");
      }
      setModalOpen(false);
      loadData();
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api(`/waschen/parfumes/${deleteTarget.id}`, { method: "DELETE" });
      showToast("Parfum berhasil dihapus");
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Total Varian Parfum</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Parfum Aktif</p>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl overflow-hidden border border-slate-200">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 bg-slate-50/50">
              <h3 className="font-bold text-slate-800 text-sm">
                {formData.id ? "Edit Aroma Parfum" : "Tambah Aroma Parfum"}
              </h3>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs">
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Nama Aroma Parfum *</label>
                <input
                  type="text"
                  placeholder="Misal: Sakura Fresh, Ocean Breeze, Lavender"
                  value={formData.name}
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Deskripsi Aroma</label>
                <textarea
                  rows={3}
                  placeholder="Penjelasan wangi aroma..."
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-sm rounded-2xl bg-white shadow-2xl p-5 border border-slate-200 space-y-4">
            <div className="flex items-center gap-3 text-rose-600">
              ...
              <h3 className="font-bold text-slate-800 text-sm">Hapus Aroma Parfum?</h3>
            </div>
            <p className="text-xs text-slate-600">
              Apakah Anda yakin ingin menghapus aroma <strong>{deleteTarget.name}</strong>? Tindakan ini tidak dapat dibatalkan.
            </p>
