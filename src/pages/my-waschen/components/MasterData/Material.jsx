...
const EMPTY_FORM = { id: null, code: "", name: "", description: "", is_active: 1 };
...
export default function Material() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const res = await api(`/waschen/materials?${query.toString()}`);
      setData(res.data || []);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.code.trim() || !formData.name.trim()) {
      setFormError("Kode dan Nama Material wajib diisi");
      return;
    }
    setSubmitting(true);
    setFormError("");
    try {
      if (formData.id) {
        await api(`/waschen/materials/${formData.id}`, { method: "PUT", body: JSON.stringify(formData) });
        showToast("Material berhasil diperbarui");
      } else {
        await api("/waschen/materials", { method: "POST", body: JSON.stringify(formData) });
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api(`/waschen/materials/${deleteTarget.id}`, { method: "DELETE" });
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <PageHero>
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-white sm:text-3xl">Master Material</h1>
            <p className="mt-3 text-sm leading-6 text-white/75">Kelola jenis bahan pakaian yang dicuci</p>
          </div>
          <button
            type="button"
            onClick={() => {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { l: "Total Material", v: stats.total, icon: HiOutlineSwatch },
          { l: "Aktif", v: stats.active, c: "text-emerald-600" },
          { l: "Nonaktif", v: stats.inactive, c: "text-slate-400" },
        ].map((s) => (
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead className="bg-slate-50/80 border-b border-slate-200 text-slate-500">
              <tr>
                ...
                <SortTh col="name" label="Nama Material" sortBy={sortBy} sortDir={sortDir} onSort={handleSort} />
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl border overflow-hidden">
              <div className="flex items-center justify-between border-b px-5 py-4 bg-slate-50/50">
                <h3 className="font-bold text-sm">{formData.id ? "Edit Material" : "Tambah Material"}</h3>
        ...
        createPortal(
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <div className="w-full max-w-sm rounded-2xl bg-white shadow-2xl p-5 space-y-4 border">
              <h3 className="font-bold text-sm">Hapus material {deleteTarget.name}?</h3>
