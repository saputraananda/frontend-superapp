...
export default function CustomerSource() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const res = await api(`/waschen/customer-sources?${query.toString()}`);
      setData(res.data || []);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.id) {
        await api(`/waschen/customer-sources/${formData.id}`, {
          method: "PUT",
          body: JSON.stringify(payload),
        });
        showToast("Sumber pelanggan berhasil diperbarui");
      } else {
        await api("/waschen/customer-sources", {
          method: "POST",
          body: JSON.stringify(payload),
        });
        showToast("Sumber pelanggan berhasil ditambahkan");
      }
      setModalOpen(false);
      loadData();
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api(`/waschen/customer-sources/${deleteTarget.id}`, { method: "DELETE" });
      showToast("Sumber pelanggan berhasil dihapus");
  ...
  const stats = useMemo(
    () => ({
      ...
      inactive: data.filter((d) => Number(d.is_active) !== 1).length,
    }),
    [data],
  );
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      {toast && (
        <div
          className={cn(
            "fixed top-5 right-5 z-50 flex items-center gap-2 rounded-xl px-4 py-3 text-xs font-semibold text-white shadow-xl",
        ...
        <div className="relative p-5 sm:p-6 lg:p-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              <h1 className="text-2xl font-bold tracking-tight text-white sm:text-3xl">Sumber Pelanggan</h1>
              <p className="mt-3 text-sm leading-6 text-white/75 sm:text-base">
                Master sumber akuisisi pelanggan
              </p>
            </div>
            <button
              type="button"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: "Total Sumber", value: stats.total, icon: HiOutlineGlobeAlt, color: "purple" },
          { label: "Aktif", value: stats.active, icon: HiOutlineCheckCircle, color: "emerald" },
          { label: "Nonaktif", value: stats.inactive, icon: HiOutlineExclamationTriangle, color: "amber" },
        ].map((s) => (
          <div
            key={s.label}
            ...
            <div>
              ...
              <p
                className={cn(
                  ...
                  s.color === "emerald" ? "text-emerald-600" : s.color === "amber" ? "text-amber-600" : "text-slate-800",
                )}
              >
                {s.value}
              </p>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="border-b border-slate-100 p-4 bg-slate-50/50 flex flex-col sm:flex-row gap-3">
          <div className="relative w-full sm:w-72">
            ...
            <input
              type="text"
              placeholder="Cari kode, nama, label..."
              value={search}
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center text-slate-400">
                    Tidak ada data sumber pelanggan
                  </td>
                </tr>
              ) : (
                data.map((item, idx) => (
            ...
            <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl overflow-hidden border border-slate-200">
              <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 bg-slate-50/50">
                <h3 className="font-bold text-slate-800 text-sm">
                  {formData.id ? "Edit Sumber Pelanggan" : "Tambah Sumber Pelanggan"}
                </h3>
                <button
                  type="button"
              ...
              <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Teks yang muncul di form pelanggan"
                    className="w-full rounded-xl border border-slate-200 px-3 py-2 text-xs outline-none focus:border-[#5f1340]"
                    required
                  />
                </div>
                ...
                <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-[#5f1340] to-[#4a0d31] px-4 py-2 text-xs font-semibold text-white shadow-md disabled:opacity-50"
                  >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <div className="w-full max-w-sm rounded-2xl bg-white shadow-2xl p-5 border border-slate-200 space-y-4">
              <div className="flex items-center gap-3 text-rose-600">
                ...
                <h3 className="font-bold text-slate-800 text-sm">Hapus Sumber Pelanggan?</h3>
              </div>
              <p className="text-xs text-slate-600">
                Hapus <strong>{deleteTarget.name}</strong> ({deleteTarget.code})? Tindakan ini tidak dapat dibatalkan.
              </p>
              <div className="flex justify-end gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-xl bg-rose-600 px-4 py-2 text-xs font-semibold text-white disabled:opacity-50"
                >
