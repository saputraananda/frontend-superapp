...
function GenderBadge({ gender }) {
  ...
  return (
    <span className={cn("inline-flex items-center rounded-full border px-2.5 py-0.5 text-[10px] font-semibold", isMale ? "border-sky-200 bg-sky-50 text-sky-700" : "border-pink-200 bg-pink-50 text-pink-700")}>
      {gender}
    </span>
  );
}
...
function toFormData(item) {
  return {
    ...
    deposit_balance: Number(item.deposit_balance) || 0,
    notes: item.notes || "",
...
export default function Customer() {
  ...
  const loadLookups = async () => {
    try {
      const [tierRes, sourceRes] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        home_branch: formData.home_branch.trim(),
        ...
        deposit_balance: Number(formData.deposit_balance) || 0,
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-3 text-sm leading-6 text-white/75 sm:text-base">
                Database pelanggan dengan tier spending & sumber akuisisi
              </p>
            </div>
            <button
              type="button"
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead className="bg-slate-50/80 border-b border-slate-200 text-slate-500">
              <tr>
                ...
                <SortTh col="gender" label="Gender" sortBy={sortBy} sortDir={sortDir} onSort={handleSort} />
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <tr key={item.id} className="hover:bg-slate-50/80">
                  ...
                  <td className="px-4 py-3.5"><GenderBadge gender={item.gender} /></td>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs overflow-y-auto">
              ...
              <FormSection title="Identitas">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div><label className="block font-semibold mb-1">Kode Pelanggan</label><input type="text" value={formData.customer_code} onChange={(e) => setFormData((p) => ({ ...p, customer_code: e.target.value.toUpperCase() }))} placeholder="Auto: CUST-XXX" className={cn(inputCls, "font-mono")} /></div>
                  ...
                  <div><label className="block font-semibold mb-1">Sapaan</label><input type="text" value={formData.greeting} onChange={(e) => setFormData((p) => ({ ...p, greeting: e.target.value }))} placeholder="Misal: Bapak / Ibu" className={inputCls} /></div>
                  ...
                  <div><label className="block font-semibold mb-1">Tanggal Lahir</label><input type="date" value={formData.birth_date} onChange={(e) => setFormData((p) => ({ ...p, birth_date: e.target.value }))} className={inputCls} /></div>
                  <div><label className="block font-semibold mb-1">Pekerjaan</label><input type="text" value={formData.occupation} onChange={(e) => setFormData((p) => ({ ...p, occupation: e.target.value }))} className={inputCls} /></div>
                </div>
              </FormSection>
              ...
              <FormSection title="Alamat">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div><label className="block font-semibold mb-1">Blok</label><input type="text" value={formData.block} onChange={(e) => setFormData((p) => ({ ...p, block: e.target.value }))} className={inputCls} /></div>
                  <div><label className="block font-semibold mb-1">No. Rumah</label><input type="text" value={formData.house_number} onChange={(e) => setFormData((p) => ({ ...p, house_number: e.target.value }))} className={inputCls} /></div>
                  <div className="sm:col-span-2"><label className="block font-semibold mb-1">Alamat Lengkap</label><textarea rows={2} value={formData.full_address} onChange={(e) => setFormData((p) => ({ ...p, full_address: e.target.value }))} className={inputCls} /></div>
                  <div><label className="block font-semibold mb-1">Kecamatan</label><input type="text" value={formData.district} onChange={(e) => setFormData((p) => ({ ...p, district: e.target.value }))} className={inputCls} /></div>
                  <div><label className="block font-semibold mb-1">Kelurahan</label><input type="text" value={formData.sub_district} onChange={(e) => setFormData((p) => ({ ...p, sub_district: e.target.value }))} className={inputCls} /></div>
                  ...
                  <div><label className="block font-semibold mb-1">Cabang Favorit</label><input type="text" value={formData.home_branch} onChange={(e) => setFormData((p) => ({ ...p, home_branch: e.target.value }))} className={inputCls} /></div>
                </div>
              </FormSection>

              <FormSection title="Tier & Deposit">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div><label className="block font-semibold mb-1">Saldo Deposit (Rp)</label><input type="number" min="0" value={formData.deposit_balance} onChange={(e) => setFormData((p) => ({ ...p, deposit_balance: e.target.value }))} className={inputCls} /></div>
                  ...
                  <div className="sm:col-span-2"><label className="block font-semibold mb-1">Catatan Umum</label><textarea rows={2} value={formData.general_notes} onChange={(e) => setFormData((p) => ({ ...p, general_notes: e.target.value }))} className={inputCls} /></div>
                </div>
                {formData.id && (
                  <p className="text-[10px] text-slate-400">Total order dan membership aktif dikelola otomatis oleh sistem POS.</p>
                )}
              </FormSection>
