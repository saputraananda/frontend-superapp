...
export default function AssignedToMe() {
  ...
  // Reviewer Filter Toggle
  const [showReviewerTasks, setShowReviewerTasks] = useState(false);

  const loadTasks = async () => {
    setLoading(true);
    try {
      const data = await api("/api/pm2/tasks/assigned");
      setTasks(data?.data || []);
    } catch (err) {
  ...
  // Dynamic Month, Year, and Position options from task data
  ...
  const filteredTasks = tasks.filter((task) => {
    // 0. Reviewer filter
    if (task.user_role === "reviewer" && !showReviewerTasks) return false;
    ...
    if (searchTerm.trim()) {
      ...
      const matchWorkspace = task.project_title?.toLowerCase().includes(q);
      const matchSubWorkspace = task.sub_workspace_title?.toLowerCase().includes(q);
      if (!matchTitle && !matchDesc && !matchPic && !matchOwner && !matchPosition && !matchWorkspace && !matchSubWorkspace) return false;
    }
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3 shrink-0">
              ...
              <div className="hidden md:flex flex-col items-end gap-1 mr-2">
                <span className="text-[10px] font-bold text-slate-500">{completedCount}/{filteredTasks.length} Selesai</span>
              ...
              {/* Show Reviewer Toggle */}
              <label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100 transition px-3 py-1.5 cursor-pointer text-xs font-bold text-slate-650">
                <input
                  type="checkbox"
                  checked={showReviewerTasks}
                  onChange={(e) => setShowReviewerTasks(e.target.checked)}
                  className="rounded text-indigo-600 focus:ring-indigo-500 border-slate-300 h-3.5 w-3.5"
                />
                Tampilkan Tugas Reviewer (CC)
              </label>
        ...
        <div className="flex flex-col md:flex-row items-center gap-3">
          <div className="relative w-full md:flex-1">
            ...
            <input
              ...
              placeholder="Cari berdasarkan judul, deskripsi, PIC, owner, position, workspace..."
          ...
          >
            <HiOutlineFunnel className="h-4 w-4 text-slate-455" />
            Filter {showFilters ? "Tutup" : "Buka"}
            {(filterPriority.length > 0 || filterStatus.length > 0 || filterPosition.length > 0 || filterMonth.length > 0 || filterYear.length > 0 || startDate || endDate || dueDateType) && (
                        ...
                        <div className="mt-3 space-y-1.5">
                          ...
                          {task.pic_name && (
                            <div className="flex items-center gap-1 text-[10px] text-slate-500">
                              ...
                              {task.user_role && (
                                <span className="ml-1 px-1 py-0.2 text-[8px] font-bold uppercase rounded bg-indigo-50 text-indigo-600 border border-indigo-100">
                                  {task.user_role}
                                </span>
                              )}
                            </div>
