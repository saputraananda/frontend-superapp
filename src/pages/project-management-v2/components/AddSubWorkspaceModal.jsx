...
export default function AddSubWorkspaceModal({ open, onClose, onSuccess, workspaceId, workspaceName }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      await api(`/api/pm2/workspaces/${workspaceId}/sub`, {
        method: "POST",
        body: JSON.stringify({
          title: form.title,
          desc: form.desc,
      ...
      setError(err.message || "Gagal membuat sub-workspace");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-black/10 overflow-hidden">
        ...
        <div className="flex items-center gap-3 border-b border-slate-100 px-6 py-4">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-sm font-bold text-slate-800">Tambah Sub-Workspace</h2>
            <p className="text-[11px] text-slate-400 truncate">
              Dalam workspace: <span className="font-semibold text-indigo-600">{workspaceName}</span>
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="flex items-center justify-end gap-2 pt-1">
            ...
            >
              {loading ? (
                ...
                <HiOutlineSquaresPlus className="h-3.5 w-3.5" />
              )}
              {loading ? "Menyimpan..." : "Buat Sub-Workspace"}
            </button>
          </div>
        </form>
      </div>
