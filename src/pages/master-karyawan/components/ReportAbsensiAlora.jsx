...
import { exportReportAbsensiAloraExcel } from "../utils/exportReportAbsensiAloraExcel";
...
export default function ReportAbsensiAlora() {
	...
	useEffect(() => {
		document.title = "Report Absensi Alora | Alora Group Indonesia";
	}, []);
	...
	const fetchAbsensi = useCallback(
		async ({ silent = false } = {}) => {
			...
			try {
				...
				const response = await api(`/alora/attendance?${qs.toString()}`);
				...
				if (!silent) {
					setError(err.message || "Gagal mengambil report absensi Alora");
					setSummary(null);
					setEmployeeSummary([]);
					setRecords([]);
				}
			...
			<div className="mx-auto max-w-screen-2xl space-y-6 px-4 sm:px-6 lg:px-8">
				<section className="relative overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-br from-blue-900 via-indigo-800 to-cyan-700 p-5 shadow-sm sm:p-6">
					...
					<div className="relative max-w-3xl">
						...
						<h1 className="mt-2 text-2xl font-bold text-white sm:text-3xl">Report Absensi Alora</h1>
						<p className="mt-2 text-sm text-white/80 sm:text-base">
							Pantau absensi karyawan Alora mobile per periode cutoff, tanpa konsep shift.
						</p>
				...
				<section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
					...
					<div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
						...
						<button
							...
							className="self-end rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
						>
							Reset
						</button>
					</div>
					...
					<div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700">
						Periode aktif: <strong>{formatDateOnly(activePeriod.startDate)}</strong> sampai{" "}
						<strong>{formatDateOnly(activePeriod.endDate)}</strong>
					</div>
				</section>
				...
				<section className="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-4">
					...
					<StatCard
						...
						subtitle="Record dengan check-in, check-out, dan foto"
						tone="emerald"
					...
					<StatCard
						...
						subtitle="Total record sudah absen masuk"
						tone="amber"
						Icon={HiOutlineClock}
					/>
				</section>
				...
				<section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
					<div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
						<div>
							<h2 className="text-base font-bold text-slate-800">Detail Riwayat Absensi Alora</h2>
							<p className="mt-0.5 text-xs text-slate-500">
								Lihat detail tanggal masuk, jam absen in/out, bukti foto, lokasi, dan status kelengkapan.
							</p>
						</div>
						...
						<table className="min-w-full text-sm">
							<thead className="border-b border-slate-100 bg-slate-50">
								<tr>
									...
									<th className="whitespace-nowrap px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Foto In</th>
							...
							<tbody className="divide-y divide-slate-100">
								...
								{!loading &&
									displayedRecords.map((row) => {
										...
										const inPhoto = resolvePhotoUrl(row.check_in_photo_url);
										const outPhoto = resolvePhotoUrl(row.check_out_photo_url);
										return (
											<tr key={row.attendance_id} className="align-top transition-colors hover:bg-blue-50/30">
												...
												<td className="px-4 py-3 text-xs">
													<div className="flex flex-col gap-1">
														...
														{row.clock_out_latitude != null && row.clock_out_longitude != null ? (
															<a
																href={`https://www.google.com/maps?q=${row.clock_out_latitude},${row.clock_out_longitude}`}
																target="_blank"
																rel="noreferrer"
																className="inline-flex items-center gap-1 text-blue-600 hover:underline"
															>
					...
					<div className="space-y-3 p-4 md:hidden">
						...
						{!loading && displayedRecords.length === 0 && (
							<p className="py-8 text-center text-sm text-slate-400">Data absensi tidak ditemukan.</p>
						)}
						{!loading &&
							displayedRecords.map((row) => {
								const duration = calcDuration(row.check_in_time, row.check_out_time);
								...
								const outPhoto = resolvePhotoUrl(row.check_out_photo_url);
								return (
									<article key={row.attendance_id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
										<div className="flex items-start justify-between gap-3">
											<div>
												...
												<p className="mt-1 text-xs text-slate-500">{formatDateOnly(row.work_date)}</p>
											</div>
											<StatusBadge label={row.status_label} />
										</div>
										<div className="mt-3 grid grid-cols-2 gap-3 text-xs text-slate-600">
											<div>
												<p className="font-semibold text-slate-400">Absen In</p>
												<p>{formatDateTime(row.check_in_time)}</p>
											</div>
											<div>
												<p className="font-semibold text-slate-400">Absen Out</p>
												<p>{formatDateTime(row.check_out_time)}</p>
											</div>
											<div>
