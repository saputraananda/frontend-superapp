...
import { HiOutlineBuildingOffice2, HiOutlineCheckCircle, HiOutlineChevronDown, HiOutlineExclamationTriangle, HiOutlineFunnel, HiOutlineMagnifyingGlass, HiOutlinePencilSquare, HiOutlineTrash, HiOutlineXCircle, HiOutlineXMark } from "react-icons/hi2";
...
export default function MasterCompany() {
	...
	useEffect(() => { document.title = "Master Company | Alora Group Indonesia"; }, []);

	const fetchItems = async () => { setLoading(true); try { const r = await api("/companies"); setItems(r.companies || []); } catch { showToast("error", "Gagal memuat data perusahaan"); } finally { setLoading(false); } };
	...
	const validate = () => {
		const e = {};
		if (!form.company_code?.trim()) e.company_code = "Kode company wajib diisi";
		if (!form.company_name?.trim()) e.company_name = "Nama company wajib diisi";
		setErrors(e); return !Object.keys(e).length;
	};

	const handleSubmit = async (e) => {
		e.preventDefault(); if (!validate()) return; setSaving(true);
		const payload = { ...form, company_code: form.company_code.toUpperCase() };
		try {
			if (editTarget) { await api(`/companies/${editTarget.company_id}`, { method: "PUT", body: JSON.stringify(payload) }); showToast("success", "Perusahaan berhasil diperbarui"); }
			else { await api("/companies", { method: "POST", body: JSON.stringify(payload) }); showToast("success", "Perusahaan berhasil ditambahkan"); }
	...
	const handleDeleteConfirm = async () => {
		...
		try { await api(`/companies/${deleteTarget.company_id}`, { method: "DELETE" }); showToast("success", "Perusahaan berhasil dihapus"); fetchItems(); }
		catch (err) { showToast("error", err?.message || "Gagal menghapus"); } finally { setConfirmOpen(false); setDeleteTarget(null); }
	};

	const filtered = items.filter((a) => {
		const s = (a.company_name + a.company_code).toLowerCase().includes(search.toLowerCase());
	...
	return (
		<div className="p-6 pb-14">
			<Toast toast={toast} />
			<ConfirmDialog open={confirmOpen} title="Hapus Perusahaan" message={`Yakin ingin menghapus perusahaan "${deleteTarget?.company_name}"?`} onConfirm={handleDeleteConfirm} onCancel={() => { setConfirmOpen(false); setDeleteTarget(null); }} />

			<div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				<div className="flex items-center gap-3">
					<div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-slate-700 to-violet-600 shadow-sm"><HiOutlineBuildingOffice2 className="h-5 w-5 text-white" /></div>
					<div><h1 className="text-base font-bold text-slate-800 leading-tight">Master Company</h1><p className="text-xs text-slate-400 mt-0.5">{items.length} perusahaan terdaftar di sistem</p></div>
				</div>
				<button onClick={openAdd} className="flex items-center gap-2 rounded-xl bg-violet-600 hover:bg-violet-700 px-4 py-2.5 text-sm font-semibold text-white shadow transition active:scale-95"><HiOutlineBuildingOffice2 className="h-4 w-4" /> Tambah Company</button>
			</div>

			<div className="mb-4 flex flex-col sm:flex-row sm:items-center gap-3">
				<div className="relative flex-1 max-w-sm"><HiOutlineMagnifyingGlass className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" /><input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari kode atau nama perusahaan..." className={cn(inputCls, "pl-10")} /></div>
				...
				<div className="fixed inset-0 z-[999] flex items-center justify-center px-4">
					...
					<div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-2xl flex flex-col">
						<div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
							<div className="flex items-center gap-2"><div className="flex items-center justify-center h-8 w-8 rounded-lg bg-violet-50"><HiOutlineBuildingOffice2 className="h-4 w-4 text-violet-600" /></div><h2 className="text-base font-bold text-slate-800">{editTarget ? "Edit Company" : "Tambah Company Baru"}</h2></div>
						...
						<form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
							<Field label="Kode Company" required error={errors.company_code}>
