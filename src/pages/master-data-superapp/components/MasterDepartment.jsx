...
const EMPTY = { company_code: "", department_name: "", is_active: true };
...
export default function MasterDepartment() {
	...
	const validate = () => {
		...
		if (!form.department_name?.trim()) e.department_name = "Nama departemen wajib diisi";
	...
	const handleSubmit = async (e) => {
		...
		try {
			if (editTarget) { await api(`/departments/${editTarget.department_id}`, { method: "PUT", body: JSON.stringify(form) }); showToast("success", "Departemen berhasil diperbarui"); }
			else { await api("/departments", { method: "POST", body: JSON.stringify(form) }); showToast("success", "Departemen berhasil ditambahkan"); }
	...
	const handleDeleteConfirm = async () => {
		...
		try { await api(`/departments/${deleteTarget.department_id}`, { method: "DELETE" }); showToast("success", "Departemen berhasil dihapus"); fetchItems(); }
		catch (err) { showToast("error", err?.message || "Gagal menghapus"); } finally { setConfirmOpen(false); setDeleteTarget(null); }
	};

	const filtered = items.filter((a) => {
		const s = (a.department_name + (a.company_code || "")).toLowerCase().includes(search.toLowerCase());
	...
	return (
		<div className="p-6 pb-14">
			<Toast toast={toast} />
			<ConfirmDialog open={confirmOpen} title="Hapus Departemen" message={`Yakin ingin menghapus departemen "${deleteTarget?.department_name}"?`} onConfirm={handleDeleteConfirm} onCancel={() => { setConfirmOpen(false); setDeleteTarget(null); }} />

			<div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				<div className="flex items-center gap-3">
					<div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-slate-700 to-violet-600 shadow-sm"><HiOutlineRectangleGroup className="h-5 w-5 text-white" /></div>
					<div><h1 className="text-base font-bold text-slate-800 leading-tight">Master Departemen</h1><p className="text-xs text-slate-400 mt-0.5">{items.length} departemen terdaftar di sistem</p></div>
				</div>
				<button onClick={openAdd} className="flex items-center gap-2 rounded-xl bg-violet-600 hover:bg-violet-700 px-4 py-2.5 text-sm font-semibold text-white shadow transition active:scale-95"><HiOutlineRectangleGroup className="h-4 w-4" /> Tambah Departemen</button>
			</div>

			<div className="mb-4 flex flex-col sm:flex-row sm:items-center gap-3">
				<div className="relative flex-1 max-w-sm"><HiOutlineMagnifyingGlass className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" /><input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari departemen..." className={cn(inputCls, "pl-10")} /></div>
			...
			<div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
				{filtered.length === 0 && !loading ? (<div className="flex flex-col items-center justify-center py-20 gap-2"><HiOutlineRectangleGroup className="h-10 w-10 text-slate-300" /><p className="text-sm text-slate-400">Tidak ada data ditemukan</p></div>) : (
					<div className="overflow-x-auto"><table className="w-full text-sm">
						<thead><tr className="border-b border-slate-100 bg-slate-50/80">{["No", "Kode Company", "Nama Departemen", "Status", "Aksi"].map((h, i) => (<th key={h} className={cn("px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider", i === 4 ? "text-right" : "text-left")}>{h}</th>))}</tr></thead>
						<tbody className="divide-y divide-slate-100">{loading ? <SkeletonRows /> : filtered.map((a, i) => (
							<tr key={a.department_id} className="hover:bg-slate-50/60 transition">
								...
								<td className="px-5 py-4 font-semibold text-slate-800">{a.department_name}</td>
				...
				<div className="fixed inset-0 z-[999] flex items-center justify-center px-4">
					...
					<div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-2xl flex flex-col">
						<div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
							<div className="flex items-center gap-2"><div className="flex items-center justify-center h-8 w-8 rounded-lg bg-violet-50"><HiOutlineRectangleGroup className="h-4 w-4 text-violet-600" /></div><h2 className="text-base font-bold text-slate-800">{editTarget ? "Edit Departemen" : "Tambah Departemen"}</h2></div>
						...
						<form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
							...
							<Field label="Nama Departemen" required error={errors.department_name}><input className={inputCls} placeholder="Finance / HR / IT" value={form.department_name} onChange={(e) => setForm({ ...form, department_name: e.target.value })} /></Field>
