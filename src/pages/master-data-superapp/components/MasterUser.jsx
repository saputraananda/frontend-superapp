...
export default function MasterUser() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			if (editTarget) {
				await api(`/users/${editTarget.id}`, { method: "PUT", body: JSON.stringify(form) });
		...
		<div className="p-6 pb-14">
			...
			<div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				<div className="flex items-center gap-3">
					...
					<div>
						<h1 className="text-base font-bold text-slate-800 leading-tight">Master User</h1>
				...
				<div className="relative" ref={dropdownRef}>
					<button
						...
						className={cn(
							...
							filterRole
								? cn("border", ROLE_BADGE[filterRole], "ring-2 ring-offset-1 ring-violet-400/30")
						...
						<div className="absolute left-0 top-full mt-2 z-50 w-44 rounded-xl border border-slate-200 bg-white shadow-lg overflow-hidden">
							<button
								...
								className={cn(
									...
									!filterRole ? "bg-violet-50 text-violet-700" : "text-slate-600 hover:bg-slate-50",
								)}
							>
							...
							{ROLES.map((r) => (
								<button
									...
									className={cn(
										"w-full flex items-center gap-2.5 px-3.5 py-2.5 text-sm font-medium transition",
										filterRole === r ? "bg-violet-50 text-violet-700" : "text-slate-600 hover:bg-slate-50",
									)}
								>
									<span className={cn("h-2 w-2 rounded-full shrink-0", ROLE_BADGE[r]?.split(" ")[0] || "bg-slate-300")} />
									{r}
									{filterRole === r && (
									...
									filtered.map((u, i) => (
										<tr key={u.id} className="hover:bg-slate-50/60 transition">
											...
											<td className="px-5 py-4 text-slate-500">
												{u.username || <span className="text-slate-300 italic">—</span>}
											</td>
						...
						<div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
							<div className="flex items-center gap-2">
								<div className="flex items-center justify-center h-8 w-8 rounded-lg bg-violet-50">
									<HiOutlineUserPlus className="h-4 w-4 text-violet-600" />
								</div>
