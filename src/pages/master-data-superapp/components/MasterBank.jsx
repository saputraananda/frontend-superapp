...
const EMPTY = { bank_name: "", is_active: true };
...
export default function MasterBank() {
	...
	const openEdit = (item) => {
		setEditTarget(item);
		setForm({ bank_name: item.bank_name, is_active: Boolean(item.is_active) });
	...
	const handleSubmit = async (e) => {
		...
		try {
			if (editTarget) {
				await api(`/banks/${editTarget.bank_id}`, { method: "PUT", body: JSON.stringify(form) });
				showToast("success", "Bank berhasil diperbarui");
			} else {
				await api("/banks", { method: "POST", body: JSON.stringify(form) });
				showToast("success", "Bank berhasil ditambahkan");
			}
	...
	const handleDeleteConfirm = async () => {
		...
		try {
			await api(`/banks/${deleteTarget.bank_id}`, { method: "DELETE" });
			showToast("success", "Bank berhasil dihapus"); fetchItems();
		} catch (err) { showToast("error", err?.message || "Gagal menghapus"); }
		finally { setConfirmOpen(false); setDeleteTarget(null); }
	};

	const filtered = items.filter((a) => {
		const s = a.bank_name.toLowerCase().includes(search.toLowerCase());
	...
	const SkeletonRows = () => (
		<>{Array.from({ length: 5 }).map((_, i) => (
			<tr key={i} className="border-t border-slate-100 animate-pulse">
				{[4, 48, 14, 20].map((w, j) => (
					<td key={j} className="px-5 py-4"><div className={`h-3.5 rounded-md bg-slate-100 w-${w}`} /></td>
				))}
			</tr>
		))}</>
	);
	...
	return (
		<div className="p-6 pb-14">
			<Toast toast={toast} />
			<ConfirmDialog open={confirmOpen} title="Hapus Bank"
				message={`Yakin ingin menghapus bank "${deleteTarget?.bank_name}"? Tindakan ini tidak dapat dibatalkan.`}
			...
			<div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				<div className="flex items-center gap-3">
					<div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-slate-700 to-violet-600 shadow-sm">
						<HiOutlineBuildingLibrary className="h-5 w-5 text-white" />
					</div>
					<div>
						<h1 className="text-base font-bold text-slate-800 leading-tight">Master Bank</h1>
						<p className="text-xs text-slate-400 mt-0.5">{items.length} bank terdaftar di sistem</p>
					</div>
				</div>
				<button onClick={openAdd}
					...
					<HiOutlineBuildingLibrary className="h-4 w-4" /> Tambah Bank
				</button>
			</div>
			...
			<div className="mb-4 flex flex-col sm:flex-row sm:items-center gap-3">
				<div className="relative flex-1 max-w-sm">
					...
					<input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
						placeholder="Cari nama bank..." className={cn(inputCls, "pl-10")} />
				</div>
				<div className="relative" ref={statusRef}>
					<button type="button" onClick={() => setStatusOpen(p => !p)}
						...
						<span>{filterStatus === "active" ? "Aktif" : filterStatus === "inactive" ? "Nonaktif" : "Filter Status"}</span>
						{filterStatus && (
							<span role="button" tabIndex={0} onClick={(e) => { e.stopPropagation(); setFilterStatus(""); }}
					...
					{statusOpen && (
						<div className="absolute left-0 top-full mt-2 z-50 w-40 rounded-xl border border-slate-200 bg-white shadow-lg overflow-hidden">
							{STATUS_OPTS.map((opt) => (
								<button key={opt.value} type="button" onClick={() => { setFilterStatus(opt.value); setStatusOpen(false); }}
									className={cn("w-full flex items-center gap-2.5 px-3.5 py-2.5 text-sm font-medium transition",
										filterStatus === opt.value ? "bg-violet-50 text-violet-700" : "text-slate-600 hover:bg-slate-50")}>
									<span className={cn("h-2 w-2 rounded-full shrink-0", opt.dot)} />{opt.label}
									{filterStatus === opt.value && <svg className="ml-auto h-3.5 w-3.5 text-violet-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>}
								</button>
							))}
						</div>
					)}
				...
				<p className="text-sm text-slate-500 shrink-0 sm:ml-auto">
					Menampilkan <span className="font-semibold text-slate-700">{filtered.length}</span> dari <span className="font-semibold text-slate-700">{items.length}</span>
				</p>
			</div>
			...
			<div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
				{filtered.length === 0 && !loading ? (
					<div className="flex flex-col items-center justify-center py-20 gap-2">
						<HiOutlineBuildingLibrary className="h-10 w-10 text-slate-300" />
						<p className="text-sm text-slate-400">Tidak ada bank ditemukan</p>
					</div>
				) : (
						...
						<table className="w-full text-sm">
							<thead>
								<tr className="border-b border-slate-100 bg-slate-50/80">
									{["No", "Nama Bank", "Status", "Aksi"].map((h, i) => (
										<th key={h} className={cn("px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider", i === 3 ? "text-right" : "text-left")}>{h}</th>
									))}
								</tr>
							</thead>
							<tbody className="divide-y divide-slate-100">
								{loading ? <SkeletonRows /> : filtered.map((a, i) => (
									<tr key={a.bank_id} className="hover:bg-slate-50/60 transition">
										<td className="px-5 py-4 text-slate-400 text-xs">{i + 1}</td>
										<td className="px-5 py-4 font-semibold text-slate-800">{a.bank_name}</td>
										...
										<td className="px-5 py-4">
											<div className="flex items-center justify-end gap-2">
												<button onClick={() => openEdit(a)} className="flex items-center gap-1.5 rounded-lg border border-violet-200 bg-violet-50 px-3 py-1.5 text-xs font-semibold text-violet-600 hover:bg-violet-100 transition">
													<HiOutlinePencilSquare className="h-3.5 w-3.5" /> Edit
												</button>
												<button onClick={() => { setDeleteTarget(a); setConfirmOpen(true); }} className="flex items-center gap-1.5 rounded-lg border border-rose-200 bg-rose-50 px-3 py-1.5 text-xs font-semibold text-rose-600 hover:bg-rose-100 transition">
					...
					<div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-2xl flex flex-col">
						<div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
							<div className="flex items-center gap-2">
								<div className="flex items-center justify-center h-8 w-8 rounded-lg bg-violet-50">
									<HiOutlineBuildingLibrary className="h-4 w-4 text-violet-600" />
								</div>
								<h2 className="text-base font-bold text-slate-800">{editTarget ? "Edit Bank" : "Tambah Bank Baru"}</h2>
							</div>
						...
						<form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
							<Field label="Nama Bank" required error={errors.bank_name}>
								<input className={inputCls} placeholder="Bank Central Asia"
							...
							<Field label="Status">
								<div className="flex items-center gap-3 h-[42px]">
									...
									<span className={cn("text-sm font-semibold", form.is_active ? "text-emerald-600" : "text-slate-400")}>
							...
							<div className="flex gap-3 pt-2">
								...
								<button type="submit" disabled={saving} className="flex-1 rounded-xl bg-violet-600 hover:bg-violet-700 disabled:opacity-60 px-4 py-2.5 text-sm font-semibold text-white transition">
									{saving ? "Menyimpan..." : editTarget ? "Simpan Perubahan" : "Tambah Bank"}
								</button>
							</div>
						</form>
					</div>
