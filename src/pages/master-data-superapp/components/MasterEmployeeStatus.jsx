...
import { HiOutlineBriefcase, HiOutlineCheckCircle, HiOutlineChevronDown, HiOutlineExclamationTriangle, HiOutlineFunnel, HiOutlineMagnifyingGlass, HiOutlinePencilSquare, HiOutlineTrash, HiOutlineXCircle, HiOutlineXMark } from "react-icons/hi2";
...
const EMPTY = { employment_status_name: "", is_active: true };
...
export default function MasterEmployeeStatus() {
	...
	useEffect(() => { document.title = "Master Status Karyawan | Alora Group Indonesia"; }, []);

	const showToast = useCallback((type, msg) => { setToast({ type, msg }); setTimeout(() => setToast(null), 3500); }, []);
	const fetchItems = useCallback(async () => { setLoading(true); try { const r = await api("/employment-statuses"); setItems(r.employeeStatuses || []); } catch { showToast("error", "Gagal memuat data status karyawan"); } finally { setLoading(false); } }, [showToast]);
	useEffect(() => { fetchItems(); }, [fetchItems]);
	const openAdd = () => { setEditTarget(null); setForm(EMPTY); setErrors({}); setModalOpen(true); };
	const openEdit = (item) => { setEditTarget(item); setForm({ employment_status_name: item.employment_status_name, is_active: Boolean(item.is_active) }); setErrors({}); setModalOpen(true); };
	const closeModal = () => { setModalOpen(false); setEditTarget(null); };
	const validate = () => { const e = {}; if (!form.employment_status_name?.trim()) e.employment_status_name = "Nama status karyawan wajib diisi"; setErrors(e); return !Object.keys(e).length; };

	const handleSubmit = async (e) => {
		e.preventDefault(); if (!validate()) return; setSaving(true);
		try {
			if (editTarget) { await api(`/employment-statuses/${editTarget.employment_status_id}`, { method: "PUT", body: JSON.stringify(form) }); showToast("success", "Status karyawan berhasil diperbarui"); }
			else { await api("/employment-statuses", { method: "POST", body: JSON.stringify(form) }); showToast("success", "Status karyawan berhasil ditambahkan"); }
	...
	const handleDeleteConfirm = async () => {
		...
		try { await api(`/employment-statuses/${deleteTarget.employment_status_id}`, { method: "DELETE" }); showToast("success", "Status karyawan berhasil dihapus"); fetchItems(); }
		catch (err) { showToast("error", err?.message || "Gagal menghapus"); } finally { setConfirmOpen(false); setDeleteTarget(null); }
	};

	const filtered = items.filter((a) => { const s = a.employment_status_name.toLowerCase().includes(search.toLowerCase()); const st = filterStatus === "active" ? a.is_active : filterStatus === "inactive" ? !a.is_active : true; return s && st; });
	const SkeletonRows = () => (<>{Array.from({ length: 5 }).map((_, i) => (<tr key={i} className="border-t border-slate-100 animate-pulse">{[4, 48, 14, 20].map((w, j) => (<td key={j} className="px-5 py-4"><div className={`h-3.5 rounded-md bg-slate-100 w-${w}`} /></td>))}</tr>))}</>);

	return (
		<div className="p-6 pb-14">
			<Toast toast={toast} />
			<ConfirmDialog open={confirmOpen} title="Hapus Status Karyawan" message={`Yakin ingin menghapus status "${deleteTarget?.employment_status_name}"?`} onConfirm={handleDeleteConfirm} onCancel={() => { setConfirmOpen(false); setDeleteTarget(null); }} />

			<div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				<div className="flex items-center gap-3">
					<div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-slate-700 to-violet-600 shadow-sm"><HiOutlineBriefcase className="h-5 w-5 text-white" /></div>
					<div><h1 className="text-base font-bold text-slate-800 leading-tight">Master Status Karyawan</h1><p className="text-xs text-slate-400 mt-0.5">{items.length} status terdaftar di sistem</p></div>
				</div>
				<button onClick={openAdd} className="flex items-center gap-2 rounded-xl bg-violet-600 hover:bg-violet-700 px-4 py-2.5 text-sm font-semibold text-white shadow transition active:scale-95"><HiOutlineBriefcase className="h-4 w-4" /> Tambah Status</button>
			</div>

			<div className="mb-4 flex flex-col sm:flex-row sm:items-center gap-3">
				<div className="relative flex-1 max-w-sm"><HiOutlineMagnifyingGlass className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" /><input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari status karyawan..." className={cn(inputCls, "pl-10")} /></div>
			...
			<div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
				{filtered.length === 0 && !loading ? (<div className="flex flex-col items-center justify-center py-20 gap-2"><HiOutlineBriefcase className="h-10 w-10 text-slate-300" /><p className="text-sm text-slate-400">Tidak ada data ditemukan</p></div>) : (
					<div className="overflow-x-auto"><table className="w-full text-sm">
						<thead><tr className="border-b border-slate-100 bg-slate-50/80">{["No", "Status Karyawan", "Status", "Aksi"].map((h, i) => (<th key={h} className={cn("px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider", i === 3 ? "text-right" : "text-left")}>{h}</th>))}</tr></thead>
						<tbody className="divide-y divide-slate-100">{loading ? <SkeletonRows /> : filtered.map((a, i) => (
							<tr key={a.employment_status_id} className="hover:bg-slate-50/60 transition">
								<td className="px-5 py-4 text-slate-400 text-xs">{i + 1}</td>
								<td className="px-5 py-4 font-semibold text-slate-800">{a.employment_status_name}</td>
				...
				<div className="fixed inset-0 z-[999] flex items-center justify-center px-4">
					...
					<div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-2xl flex flex-col">
						<div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
							<div className="flex items-center gap-2"><div className="flex items-center justify-center h-8 w-8 rounded-lg bg-violet-50"><HiOutlineBriefcase className="h-4 w-4 text-violet-600" /></div><h2 className="text-base font-bold text-slate-800">{editTarget ? "Edit Status Karyawan" : "Tambah Status Karyawan"}</h2></div>
						...
						<form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
							<Field label="Nama Status Karyawan" required error={errors.employment_status_name}><input className={inputCls} placeholder="Karyawan Tetap" value={form.employment_status_name} onChange={(e) => setForm({ ...form, employment_status_name: e.target.value })} /></Field>
