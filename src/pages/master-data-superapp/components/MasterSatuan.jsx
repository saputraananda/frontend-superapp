...
const EMPTY = { satuan_name: "", is_active: true };
...
export default function MasterSatuan() {
	...
	useEffect(() => { document.title = "Master Satuan | Alora Group Indonesia"; }, []);

	const showToast = useCallback((type, msg) => { setToast({ type, msg }); setTimeout(() => setToast(null), 3500); }, []);
	const fetchItems = useCallback(async () => {
		setLoading(true);
		try { const r = await api("/satuan"); setItems(r.satuan || []); }
		catch { showToast("error", "Gagal memuat data satuan"); }
		finally { setLoading(false); }
	}, [showToast]);
	useEffect(() => { fetchItems(); }, [fetchItems]);
	const openAdd = () => { setEditTarget(null); setForm(EMPTY); setErrors({}); setModalOpen(true); };
	const openEdit = (item) => { setEditTarget(item); setForm({ satuan_name: item.satuan_name, is_active: Boolean(item.is_active) }); setErrors({}); setModalOpen(true); };
	const closeModal = () => { setModalOpen(false); setEditTarget(null); };
	const validate = () => { const e = {}; if (!form.satuan_name?.trim()) e.satuan_name = "Nama satuan wajib diisi"; setErrors(e); return !Object.keys(e).length; };

	const handleSubmit = async (e) => {
		e.preventDefault(); if (!validate()) return; setSaving(true);
		try {
			if (editTarget) { await api(`/satuan/${editTarget.satuan_id}`, { method: "PUT", body: JSON.stringify(form) }); showToast("success", "Satuan berhasil diperbarui"); }
			else { await api("/satuan", { method: "POST", body: JSON.stringify(form) }); showToast("success", "Satuan berhasil ditambahkan"); }
	...
	const handleDeleteConfirm = async () => {
		...
		try { await api(`/satuan/${deleteTarget.satuan_id}`, { method: "DELETE" }); showToast("success", "Satuan berhasil dihapus"); fetchItems(); }
		catch (err) { showToast("error", err?.message || "Gagal menghapus"); }
		finally { setConfirmOpen(false); setDeleteTarget(null); }
	};

	const filtered = items.filter((a) => {
		const s = a.satuan_name.toLowerCase().includes(search.toLowerCase());
	...
	return (
		<div className="p-6 pb-14">
			<Toast toast={toast} />
			<ConfirmDialog open={confirmOpen} title="Hapus Satuan"
				message={`Yakin ingin menghapus satuan "${deleteTarget?.satuan_name}"? Tindakan ini tidak dapat dibatalkan.`}
			...
			<div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				<div className="flex items-center gap-3">
					<div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-slate-700 to-violet-600 shadow-sm">
						<HiOutlineRectangleGroup className="h-5 w-5 text-white" />
					</div>
					<div>
						<h1 className="text-base font-bold text-slate-800 leading-tight">Master Satuan</h1>
						<p className="text-xs text-slate-400 mt-0.5">{items.length} satuan terdaftar di sistem</p>
					</div>
				</div>
				<button onClick={openAdd} className="flex items-center gap-2 rounded-xl bg-violet-600 hover:bg-violet-700 px-4 py-2.5 text-sm font-semibold text-white shadow transition active:scale-95">
					<HiOutlineRectangleGroup className="h-4 w-4" /> Tambah Satuan
				</button>
			</div>
			...
			<div className="mb-4 flex flex-col sm:flex-row sm:items-center gap-3">
				<div className="relative flex-1 max-w-sm">
					...
					<input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari nama satuan..." className={cn(inputCls, "pl-10")} />
				</div>
			...
			<div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
				{filtered.length === 0 && !loading ? (
					<div className="flex flex-col items-center justify-center py-20 gap-2"><HiOutlineRectangleGroup className="h-10 w-10 text-slate-300" /><p className="text-sm text-slate-400">Tidak ada satuan ditemukan</p></div>
				) : (
					<div className="overflow-x-auto">
						<table className="w-full text-sm">
							<thead><tr className="border-b border-slate-100 bg-slate-50/80">{["No", "Nama Satuan", "Status", "Aksi"].map((h, i) => (<th key={h} className={cn("px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider", i === 3 ? "text-right" : "text-left")}>{h}</th>))}</tr></thead>
							<tbody className="divide-y divide-slate-100">
								{loading ? <SkeletonRows /> : filtered.map((a, i) => (
									<tr key={a.satuan_id} className="hover:bg-slate-50/60 transition">
										<td className="px-5 py-4 text-slate-400 text-xs">{i + 1}</td>
										<td className="px-5 py-4 font-semibold text-slate-800">{a.satuan_name}</td>
				...
				<div className="fixed inset-0 z-[999] flex items-center justify-center px-4">
					...
					<div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-2xl flex flex-col">
						<div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
							<div className="flex items-center gap-2"><div className="flex items-center justify-center h-8 w-8 rounded-lg bg-violet-50"><HiOutlineRectangleGroup className="h-4 w-4 text-violet-600" /></div><h2 className="text-base font-bold text-slate-800">{editTarget ? "Edit Satuan" : "Tambah Satuan Baru"}</h2></div>
						...
						<form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
							<Field label="Nama Satuan" required error={errors.satuan_name}>
								<input className={inputCls} placeholder="Pcs" value={form.satuan_name} onChange={(e) => setForm({ ...form, satuan_name: e.target.value })} />
							</Field>
							<Field label="Status">
								<div className="flex items-center gap-3 h-[42px]">
									<button type="button" onClick={() => setForm({ ...form, is_active: !form.is_active })} className={cn("relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors", form.is_active ? "bg-emerald-500" : "bg-slate-200")}><span className={cn("inline-block h-5 w-5 rounded-full bg-white shadow-sm transition-transform", form.is_active ? "translate-x-5" : "translate-x-0")} /></button>
							...
							<div className="flex gap-3 pt-2">
								...
								<button type="submit" disabled={saving} className="flex-1 rounded-xl bg-violet-600 hover:bg-violet-700 disabled:opacity-60 px-4 py-2.5 text-sm font-semibold text-white transition">{saving ? "Menyimpan..." : editTarget ? "Simpan Perubahan" : "Tambah Satuan"}</button>
							</div>
						</form>
					</div>
				</div>
