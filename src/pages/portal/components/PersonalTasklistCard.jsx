...
export default function PersonalTasklistCard() {
    ...
    return (
        <div className="flex flex-col">
            {/* ── Header ── */}
            <div className="border-b border-slate-100 bg-slate-50 px-4 sm:px-5 py-4">
                {/* title row */}
                <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-3 min-w-0">
                        <div className="flex items-center justify-center h-9 w-9 rounded-lg bg-violet-100 shrink-0">
                            <IconClipboard />
                        </div>
                        <div className="min-w-0">
                            <h3 className="text-sm font-bold text-slate-800 truncate">Personal Tasklist</h3>
                    ...
                    <button
                        onClick={openCreate}
                        className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg bg-violet-600 text-white hover:bg-violet-700 transition"
                    >
                        <IconPlus /> Buat
                    </button>
                </div>
                {/* progress bar — always on its own row */}
                {totalAll > 0 && (
            ...
            {/* ── Filter Bar ── */}
            <div className="px-4 py-3 border-b border-slate-100 bg-white flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-2 sm:gap-3">
                <select
                    value={filterCompanyId}
                    onChange={(e) => setFilterCompanyId(e.target.value)}
                    className="w-full sm:w-auto rounded-lg border border-slate-200 bg-white px-3 py-2 sm:py-1.5 text-xs text-slate-700 outline-none focus:ring-2 focus:ring-violet-500/30 focus:border-violet-400"
                >
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    <div className="flex items-center gap-1.5 flex-1 sm:flex-none">
                        <span className="text-[10px] text-slate-400 font-semibold shrink-0">Dari</span>
                        <input
                            type="date"
                            value={filterDateFrom}
                            onChange={(e) => setFilterDateFrom(e.target.value)}
                            className="flex-1 sm:flex-none rounded-lg border border-slate-200 bg-white px-2.5 py-2 sm:py-1.5 text-xs text-slate-700 outline-none focus:ring-2 focus:ring-violet-500/30 focus:border-violet-400"
                        />
                    </div>
                    <div className="flex items-center gap-1.5 flex-1 sm:flex-none">
                        <span className="text-[10px] text-slate-400 font-semibold shrink-0">Sampai</span>
                        <input
                            type="date"
                            value={filterDateTo}
                ...
                {(filterCompanyId || filterDateFrom || filterDateTo) && (
                    <button
                        ...
                        className="text-[10px] text-rose-500 font-semibold hover:underline self-start sm:self-auto"
                    >
                        Reset Filter
                    </button>
                )}
            ...
            {/* ── Body ── */}
            <div className="flex-1 overflow-y-auto max-h-[60vh] sm:max-h-[480px]">
                {loading ? (
    ...
    return (
        <div>
            ...
            <button
                ...
                className="w-full flex items-start gap-3 px-4 sm:px-5 py-3.5 text-left hover:bg-slate-50/60 active:bg-slate-50 transition group"
            >
                {/* chevron — aligned to first line */}
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                        ...
                        {list.assignees?.length > 0 && (
                            <span className="shrink-0 flex items-center gap-1 text-[10px] text-slate-400 bg-slate-100 rounded-full px-2 py-0.5">
                ...
                {isOwner && (
                    <div className="shrink-0 flex items-center gap-1 mt-0.5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition">
                        <span
                    ...
                    {list.assignees?.length > 0 && (
                        <div className="flex flex-wrap items-center gap-1.5 mb-3">
                            ...
                            {list.assignees.map((a) => (
                                <span
                                    key={a.employee_id}
                                    className="inline-flex items-center gap-1 bg-violet-50 text-violet-700 text-[10px] font-medium rounded-full px-2 py-0.5 border border-violet-100"
                                >
                                    {a.full_name}
                                </span>
                            ))}
            ...
            {item.evidence_type === "none" && (
                <>
                    ...
                    <button
                        ...
                        disabled={uploading}
                        className="shrink-0 p-1.5 rounded-lg text-slate-400 hover:text-violet-500 hover:bg-violet-50 transition"
...
function TasklistModal({ editingList, onClose, onSaved }) {
    ...
    const validate = () => {
        ...
        if (items.filter((it) => it.content.trim()).length === 0) e.items = "Minimal 1 item checklist";
        return e;
    };
            ...
            <div className="relative z-10 w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col" style={{ maxHeight: "90vh" }}>
                ...
                <div className="flex items-center justify-between border-b border-slate-100 px-4 sm:px-6 py-4 shrink-0">
                    <div className="flex items-center gap-2 min-w-0">
                        <div className="flex items-center justify-center h-8 w-8 rounded-lg bg-violet-50 shrink-0">
                            <IconClipboard />
                        </div>
                    ...
                    <button onClick={onClose} className="shrink-0 p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition"><IconX /></button>
                </div>
                ...
                {/* Footer */}
                <div className="border-t border-slate-100 px-4 sm:px-6 py-4 flex gap-3 shrink-0">
