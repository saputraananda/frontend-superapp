...
const MENU_ITEMS = [
    {
        ...
        label: "Dashboard",
        description: "Ringkasan training",
        end: true,
    },
    {
        to: "/training-management-system/request",
        icon: HiOutlineDocumentText,
        label: "Pengajuan Training",
        description: "Request & approval training",
    },
];
    ...
    return (
        <NavLink
            ...
            style={({ isActive }) => isActive
                ? { background: "linear-gradient(135deg, #E8823A, #D4A12A)", boxShadow: "0 4px 14px rgba(232,130,58,0.35)" }
                : {}
            }
        >
            {({ isActive }) => (
        ...
        <div className="flex h-full flex-col overflow-hidden">
            <div className={cn("flex items-center border-b border-slate-100 py-4",
                ...
                {!collapsed && (
                    <div className="flex min-w-0 flex-1 items-center gap-3">
                        ...
                        <div className="min-w-0">
                            <p className="text-sm font-bold text-slate-800 leading-tight truncate">Training System</p>
                ...
                {collapsed && (
                    <div
                        className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl shadow-md"
                        style={{ background: "linear-gradient(135deg, #F55555, #FCCF31)" }}
                    >
                        <HiOutlineAcademicCap className="h-5 w-5 text-white" />
                    </div>
                )}
...
function ActiveMenuTitle() {
    ...
    const label       = active?.label       ?? "Training Management";
    const description = active?.description ?? "Sistem manajemen pelatihan";
    const Icon        = active?.icon        ?? HiOutlineAcademicCap;
    return (
        <div className="flex items-center gap-2.5">
            <div
                className="flex h-8 w-8 items-center justify-center rounded-lg shrink-0"
                style={{ background: "linear-gradient(135deg, #E8823A20, #D4A12A20)", border: "1px solid #D4A12A60" }}
            >
                <Icon className="h-4 w-4" style={{ color: "#E8823A" }} />
            </div>
            <div>
...
export default function TrainingManagementSystem() {
    ...
    useEffect(() => {
        document.title = "Training Management System | Alora Group Indonesia";
            ...
            <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
                <header className="hidden lg:flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4">
                    ...
                    <div
                        ...
                        style={{ background: "#FFFAE8", border: "1px solid #D4A12A80", color: "#A07D18" }}
                    >
                        <div className="h-2 w-2 rounded-full" style={{ background: "#D4A12A" }} />
                        Training Management
                    </div>
                </header>
                ...
                <header className="flex items-center gap-3 border-b border-slate-200 bg-white px-4 py-3 lg:hidden">
                    ...
                    <div className="flex items-center gap-2">
                        <div
                            ...
                            style={{ background: "linear-gradient(135deg, #E8823A, #D4A12A)" }}
                        >
                            <HiOutlineAcademicCap className="h-3.5 w-3.5 text-white" />
                        </div>
                        <span className="text-sm font-bold text-slate-700">Training System</span>
                    </div>
                </header>
