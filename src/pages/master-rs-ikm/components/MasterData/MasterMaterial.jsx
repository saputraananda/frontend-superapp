...
const EMPTY = { material_code: "", material_name: "", description: "", is_active: 1 };
...
export default function MasterMaterial() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try { setData(await api("/ikm/master-data/materials") || []); }
  ...
  const openEdit = (r) => { setEdit(r); setForm({ material_code: r.material_code, material_name: r.material_name, description: r.description || "", is_active: r.is_active ?? 1 }); setModal(true); };

  const handleSave = async (e) => {
    e.preventDefault();
    if (!form.material_code || !form.material_name) { showToast("error", "Kode & nama wajib diisi"); return; }
    setSaving(true);
    try {
      if (edit) { await api(`/ikm/master-data/materials/${edit.id}`, { method: "PUT", body: JSON.stringify(form) }); showToast("success", "Bahan diperbarui"); }
      else { await api("/ikm/master-data/materials", { method: "POST", body: JSON.stringify(form) }); showToast("success", "Bahan ditambahkan"); }
  ...
  const handleDelete = async () => {
    if (!del) return;
    try { await api(`/ikm/master-data/materials/${del.id}`, { method: "DELETE" }); showToast("success", "Bahan dihapus"); setDel(null); fetchData(); }
    catch (err) { showToast("error", err?.message || "Gagal"); }
  };

  const filtered = data.filter(r =>
    (r.material_code + " " + r.material_name + " " + (r.description || "")).toLowerCase().includes(search.toLowerCase())
  );
    ...
    <div className="p-6 pb-14">
      ...
      {del && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center px-4" onClick={() => setDel(null)}>
          ...
          <div className="relative z-10 w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-base font-bold text-slate-800">Hapus Bahan</h3>
            <p className="mt-1 text-sm text-slate-500 mb-5">Hapus bahan <span className="font-semibold">{del.material_name}</span>?</p>
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-slate-800 leading-tight">Master Bahan</h1>
            <p className="text-xs text-slate-400 mt-0.5">{data.length} bahan terdaftar</p>
          </div>
        </div>
        <button onClick={openAdd} className="flex items-center gap-2 rounded-xl bg-red-600 hover:bg-red-700 px-4 py-2.5 text-sm font-semibold text-white shadow transition active:scale-95">
          <HiOutlinePlus className="h-4 w-4" /> Tambah Bahan
        </button>
      </div>

      <div className="mb-4 relative max-w-sm">
        <HiOutlineMagnifyingGlass className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
        <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Cari bahan..." className={cn(inputCls, "pl-10")} />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-2 text-slate-400"><HiOutlineCircleStack className="h-10 w-10" /><p className="text-sm">Belum ada bahan</p></div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/80">
                  {["No", "Kode", "Nama Bahan", "Deskripsi", "Status", "Aksi"].map((h, i) => (
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((r, i) => (
                  <tr key={r.id} className="hover:bg-slate-50/60 transition">
                    ...
                    <td className="px-4 py-4"><span className="inline-flex items-center px-2 py-0.5 rounded-md bg-red-50 text-red-700 text-xs font-mono font-semibold">{r.material_code}</span></td>
                    <td className="px-4 py-4 font-semibold text-slate-800">{r.material_name}</td>
                    <td className="px-4 py-4 text-slate-500 text-xs max-w-[200px] truncate">{r.description || <span className="text-slate-300">—</span>}</td>
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center px-4">
          ...
          <div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
              <h2 className="text-base font-bold text-slate-800">{edit ? "Edit Bahan" : "Tambah Bahan"}</h2>
            ...
            <form onSubmit={handleSave} className="px-6 py-5 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-xs font-semibold text-slate-600 mb-1">Kode <span className="text-rose-500">*</span></label><input className={inputCls} placeholder="KATUN" value={form.material_code} onChange={e => setForm({ ...form, material_code: e.target.value.toUpperCase() })} required /></div>
                <div><label className="block text-xs font-semibold text-slate-600 mb-1">Nama <span className="text-rose-500">*</span></label><input className={inputCls} placeholder="Katun" value={form.material_name} onChange={e => setForm({ ...form, material_name: e.target.value })} required /></div>
              </div>
